...
//    v.3.57  - UnitTest_SetName() problem with leak inside unit test
//    v.3.58  - GetCRC32 from buffer
//----------------------------------------------------------------------------- 

#ifndef FRAMEWORK_H
#define FRAMEWORK_H

#define FRAMEWORK_VER "3.58"
...
#define UNIT_TEST_CPP_H
		...
		#define UNIT_TEST_MEMORYCHECK_END	  } \
											  int UnitTest_MemLeakCount_End = MemTrace::LeakCount(); \
...
#pragma warning(pop)
		...
		public:
			...
			static void RunTests()
			{
				...
				UnitTrace::out2("\n");
				UnitTrace::out2("      Ignored: %d\n", unitStats.testDisabled);
				UnitTrace::out2("       Passed: %d\n", unitStats.testPass);
				UnitTrace::out2("       Failed: %d\n", unitStats.testFail);
				UnitTrace::out2("\n");
		...
		public:
			enum Flag
			{
				...
				ALL = MEMORY_CHECK | VERSION_CHECK,
				NONE = 0
			};
...
#define FILE_MODIFIED_CHECK(A,B) { uint32_t file_crc; \
...
#define FILE_GET_CRC32(A) CRC32::GetCRC(A)
		...
		public:
			...
			static uint32_t GetCRC(const void *pBuff, size_t numBytes)
			{
				...
				pCRC->privGetCRCFromBuff(pBuff, numBytes, CRC32_result);
			...
			static bool GetCRC(const char *const pFileName, uint32_t &crc)
			{
				...
				else
				{
					...
					{
						pCRC->privGetCRCFromBuff(buf, fread(buf, 1, sizeof(buf), pFile), crc);
					}
		...
		private:
			...
			void privGetCRCFromBuff(const void *const data, const size_t n_bytes, uint32_t &crc)
			{
